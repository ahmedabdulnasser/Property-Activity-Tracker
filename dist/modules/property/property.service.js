...
let PropertyService = class PropertyService {
    ...
    async deleteAll() {
        return this.propertyRepository.deleteAll();
    }
    ...
    async seedFromCsv(csvPath) {
        ...
        for (const record of records) {
            ...
            const latitude = record.latitude || record.Latitude || 0;
            const longitude = record.longitude || record.Longitude || 0;
            if (!address)
                continue;
            const propertyName = address.split(",")[0].trim();
            await this.create({ propertyName, address, longitude, latitude });
            count++;
        }
        return count;
    }
